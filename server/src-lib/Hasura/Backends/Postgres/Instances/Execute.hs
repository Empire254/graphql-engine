...
import Hasura.Server.Types (TraceQueryStatus (TraceQueryEnabled))
...
convertFunction sourceName modelSourceType userInfo traceQueryStatus jsonAggSelect unpreparedQuery = do
  ...
  pure
    $! ( fst
           $ mkCurPlanTx userInfo preparedSQLWithQueryTags traceQueryStatus, -- forget (Maybe PreparedSql)
         modelNames
       )
...
pgDBMutationPlan ::
  ...
  TraceQueryStatus ->
  m (DBStepInfo ('Postgres pgKind), [ModelInfoPart])
pgDBMutationPlan env manager logger userInfo stringifyNum sourceName sourceConfig mrf reqHeaders operationName selSetArguments traceQueryStatus = do
  resolvedConnectionTemplate <-
    let connectionTemplateResolver =
          connectionTemplateConfigResolver (_pscConnectionTemplateConfig sourceConfig)
        queryContext =
